...
单位内部使用的假期值班报名网页：员工在公网填报，后台自动汇总统计。
...
| 重复报名 | 同一员工同一日期只能报一个班次 | 同一员工同一日期只能报名一个班次，××× 已报名×月×日的×班 |
...
- 员工身份靠「姓名 + 手机号」识别，没有对接统一认证。知道别人姓名和手机号的人可以替别人取消报名；
  如需更严格的身份校验，可接单位统一登录或短信验证码。
- 报名页公开显示**姓名和科室**（不显示手机号）。如果单位要求匿名，需要改成只显示人数。
