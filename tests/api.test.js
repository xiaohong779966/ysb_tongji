...
const ME = { name: '张三', dept: '办公室', phone: '13800000001' };
...
(async function run() {
  ...
  console.log('\n[2] 正常报名');
  const r1 = await signup({ date: '2026-09-25', shift: '中班', name: ME.name, dept: ME.dept, phone: ME.phone });
  ok(r1.status === 200 && r1.body.ok, '张三报名 9月25日 中班成功', r1.body);
  const r2 = await signup({ date: '2026-09-25', shift: '中班', name: '李四', phone: '13800000002' });
  ok(r2.status === 200 && r2.body.ok, '李四报名 9月25日 中班成功（中班满 2 人）');

  console.log('\n[3] 班次已满');
  const r3 = await signup({ date: '2026-09-25', shift: '中班', name: '王五', phone: '13800000003' });
  ...
  const r4 = await signup({ date: '2026-09-25', shift: '晚班', name: '王五', phone: '13800000003' });
  ok(r4.status === 200 && r4.body.ok, '王五改报晚班成功（此时当天 3 人已满）');

  console.log('\n[4] 当天已满');
  const r5 = await signup({ date: '2026-09-25', shift: '中班', name: '赵六', phone: '13800000006' });
  ...
  console.log('\n[5] 同一人同一天只能报一个班次');
  const r6 = await signup({ date: '2026-09-25', shift: '晚班', name: ME.name, phone: ME.phone });
  ok(r6.status === 409 && r6.body.code === 'DUPLICATE', '张三同日再报晚班被拒绝', r6.body);
  const r7 = await signup({ date: '2026-10-01', shift: '晚班', name: ME.name, dept: ME.dept, phone: ME.phone });
  ok(r7.status === 200 && r7.body.ok, '张三换一天（10月1日）可以报名');

  console.log('\n[6] 输入校验');
  const b1 = await signup({ date: '2026-12-25', shift: '中班', name: '测试', phone: '13800000009' });
  ok(b1.status === 400 && b1.body.code === 'BAD_DATE', '不在安排内的日期被拒绝');
  const b2 = await signup({ date: '2026-09-26', shift: '夜班', name: '测试', phone: '13800000009' });
  ok(b2.status === 400 && b2.body.code === 'BAD_SHIFT', '不存在的班次被拒绝');
  const b3 = await signup({ date: '2026-09-26', shift: '中班', name: '测试', phone: '123' });
  ok(b3.status === 400 && b3.body.code === 'BAD_PHONE', '手机号格式错误被拒绝');
  const b4 = await signup({ date: '2026-09-26', shift: '中班', name: '   ', phone: '13800000009' });
  ...
  ok(day.中班[0].phone === undefined, '公开接口不返回手机号');
  ok(day.中班[0].name === '张三', '公开接口返回姓名');

  console.log('\n[8] 我的报名 / 取消');
  const mine = await post('/api/my', { name: ME.name, phone: ME.phone });
  ok(mine.body.list.length === 2, '张三查到 2 条报名', mine.body.list);
  const wrong = await post('/api/cancel', { id: mine.body.list[0].id, name: ME.name, phone: '13800000000' });
  ok(wrong.status === 403, '手机号不符不能取消报名');
  const cancel = await post('/api/cancel', { id: mine.body.list[0].id, name: ME.name, phone: ME.phone });
  ...
  ok(/^1[3-9]\d{9}$/.test(aDay.中班[0].phone || ''), '管理接口能看到手机号：' + aDay.中班[0].phone);
  ...
  const reAdd = await post('/api/admin/add', { date: '2026-09-25', shift: '中班', name: '陈七', dept: '财务科', phone: '13800000007' }, { Cookie: cookie });
  ok(reAdd.body.ok, '管理员代报名成功（中班满 2 人）');
  const overShift = await post('/api/admin/add', { date: '2026-09-25', shift: '中班', name: '新人', phone: '13800000008' }, { Cookie: cookie });
  ok(overShift.body.code === 'SHIFT_FULL' && overShift.body.message === '该班次报名人数已满，请选择其他班次', '代报名同样受班次上限约束');
  const fillLate = await post('/api/admin/add', { date: '2026-09-25', shift: '晚班', name: '王五', phone: '13800000003' }, { Cookie: cookie });
  ok(fillLate.body.ok, '补报晚班成功（当天 3 人满员）');
  const overDate = await post('/api/admin/add', { date: '2026-09-25', shift: '晚班', name: '新人', phone: '13800000008' }, { Cookie: cookie });
